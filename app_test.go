...
func TestAppServeHTTPError(t *testing.T) {
	...
	app.GET("/nok", func(ctx *Context) {
		ctx.ServeError(http.StatusInternalServerError, errors.New("nok error"))
	})
	...
	if rr.Code != http.StatusInternalServerError {
		t.Errorf("handler returned wrong status code: got %v expected %v", rr.Code, http.StatusInternalServerError)
	}
